 ...
 *
 * Using a [Lifecycle.Observer] to listen for when [LifecycleEventEffect] enters
...
public fun LifecycleEventEffect(
  ...
  DisposableEffect(lifecycleOwner) {
    val cancellable = lifecycleOwner.lifecycle.subscribe { e ->
      if (e == event) {
        currentOnEvent()
      }
    }
 ...
 *
 * This function uses a [Lifecycle.Observer] to listen for when [LifecycleStartEffect]
 ...
 * * Unique request parameters to an operation that must cancel and retry if those parameters change
 *
 * This function uses a [Lifecycle.Observer] to listen for when [LifecycleResumeEffect]
 * enters the composition and the effects will be launched when receiving a
